(() => {
    ...
    const lenTable = [256, 256, 128, 128, 64, 32, 16, 8];
            ...
            for (let sample = 0; sample < leftBuffer.length; sample++) {
                ...
                for (let i = 0; i < 16; i++) {
                    if (this.state[i].playing) {
                        const samples = (i < 8) ? lenTable[this.state[i].octave] : drums[i - 8].samples;

                        this.state[i].t += (this.state[i].frequency / this.sampleRate);
                        ...
                        const t = this.state[i].t | 0;
                        let pos = (t % samples) * advTable[this.state[i].octave];
                        let pos2 = !this.looping && t == samples ?
                            pos
                            : ((t + 1) % samples) * advTable[this.state[i].octave];
                        const s1 = i < 8
                        ...
                        if (pan < 0) {
                            right = Math.pow(10, pan / 2000) * 4;
                        } else if (pan > 0) {
                            left = Math.pow(10, -pan / 2000) * 4;
                        }
    ...
    window.initOrganya = async () => {
        ...
        const res = await fetch("player/wavetable.bin");
